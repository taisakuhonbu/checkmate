...
HashBase::HashStatus SHA1Sum::Reset()
{
    Length_Low             = 0;
    Length_High            = 0;
 ...
 *
 */
HashBase::HashStatus SHA1Sum::Result(uint8_t * const Message_Digest)
{
    int i;

    if(Message_Digest == (uint8_t *)0)
    {
        return Null;
    }
...
HashBase::HashStatus SHA1Sum::Input(const uint8_t *message_array, unsigned int length)
{
    if(length == 0)
    {
        return Success;
 ...
 *
 */
void SHA1Sum::SHA1ProcessMessageBlock()
{
